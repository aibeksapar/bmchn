...
export class FullPage {
	constructor(element, options) {
		let config = {
			...
			bulletsClass: window.matchMedia("(max-width: 375px)").matches ?  'select__options' : 'fp-bullets',
			// Класс буллета
			bulletClass: window.matchMedia("(max-width: 375px)").matches ? 'select__option' : 'fp-bullet',
			// Класс активного буллета
			bulletActiveClass: window.matchMedia("(max-width: 375px)").matches ? '_select-selected' : 'fp-bullet-active',
			//===============================
			// События
			// Событие создания
			onInit: function () { },
	...
	setBullets() {
		...
		// Если нету создаем
		/// создание селекта (моб. разрешение)
		if (!this.bulletsWrapper) {
		...
		if (this.bulletsWrapper) {
			for (let index = 0; index < this.sections.length; index++) {
				...
				if (!window.matchMedia("(max-width: 375px)").matches) {
					const btnBullet = document.createElement('span');

					btnBullet.classList.add('fp-bullet');
					...
					switch (index) {
						...
						case 4:
							...
							break;
						default:
							btnBullet.classList.add('nav__title');
					}
					this.bulletsWrapper.append(btnBullet);
				}
			}
		}
	}
